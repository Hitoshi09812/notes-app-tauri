...
pub(crate) async fn check_database_exists(db_url: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::Sqlite::database_exists(db_url).await?;
    Ok(result)
}
...
pub(crate) async fn insert_note(pool: &SqlitePool, note: Note) -> DbResult<()> {
    // // トランザクションを開始する
